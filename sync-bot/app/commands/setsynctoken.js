...
export const data = new SlashCommandBuilder()
	...
	.addStringOption((option) =>
		option
			.setName("token")
			.setDescription("The token specified in the module's `config.php` file.")
			.setRequired(true)
	);
			...
			if (guild === null) {
				...
				if (guild.guild_id !== interaction.guild.id) {
					...
					guild
						...
						.then(() => interaction.editReply("Sync token assigned to this guild."));
				} else {
					interaction.editReply("That sync token is already assigned to this guild.");
				}
			}
		});
	});
